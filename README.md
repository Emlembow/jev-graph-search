...
```sh
npx --yes --package=github:Emlembow/jevgraph jevgraph setup
```
...
```sh
npx --yes --package=github:Emlembow/jevgraph jevgraph search "Why did we choose this database?" --input ./memory
```

For a persistent `jevgraph` command:

```sh
npm install --global github:Emlembow/jevgraph
jevgraph --help
```

These commands install from GitHub; they do not require an npm registry release.

<details>
<summary>Try the included example without a key</summary>

```sh
git clone https://github.com/Emlembow/jevgraph.git
cd jevgraph
